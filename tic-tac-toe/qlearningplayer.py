...
class QLearningPlayer:
    ...
    def fit(self, opponent, m, seed=None):
        """ learning q function.
        ...
        for j in range(m):
            ...
            for k in range(self.T):
                ...
                if is_finished:
                    self.update_q(encoded, action, reward, next_state, True)
                    break
    ...
    def update_q(self, state, action, reward, next_state, is_finished):
        if is_finished:
            self.q[state][action] += self._lr * \
